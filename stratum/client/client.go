...
type Client struct {
	destination  string
	conn         net.Conn
	ResponseChan chan *rpc.Response
...
func (cl *Client) Connect(
	...
	if err != nil {
		kilolog.Warn("Connection failed:", err)
	...
	cl.ResponseChan = make(chan *rpc.Response)
	cl.alive = true
	...
	go cl.dispatchJobs(cl.conn, jc, response.Result.Job, cl.ResponseChan)
	return jc, nil
}
...
func (cl *Client) dispatchJobs(conn net.Conn, jobChan chan<- *rpc.CompleteJob, firstJob *rpc.CompleteJob, responseChan chan<- *rpc.Response) {
	...
	for {
		...
		if response.Method != "job" {
			kilolog.Debug("Received non-job response", response, "error is", response.Error)
