...
func main() {
	...
	if config.CFG.Title {
		...
		kilolog.Printf("%s * %s%s\n", bold+colGreen, colWhite,
			"VERSION      "+colCyan+"Kilonevo"+colWhite+" v"+config.VERSION.ToString())
		kilolog.Printf("%s * %s%s\n", bold+colGreen, colWhite,
...
var nevoRegexp = regexp.MustCompile("^N[1-9A-HJ-NP-Za-km-z]+$")

func configurator() {
	userAddr := prompt(kilolog.COLOR_CYAN + "Enter your wallet address: " + kilolog.COLOR_WHITE)
	...
	curcfg = strings.ReplaceAll(curcfg, "PORT_TLS", "7778")
	curcfg = strings.ReplaceAll(curcfg, "PORT_NO_TLS", "7777")
